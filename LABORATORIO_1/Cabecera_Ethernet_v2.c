...
void analizar_trama(unsigned char cabecera[36]){
    // if(tipo == 'T'){
        ...
        /*
        Recordar que para saber el tipo de tu trama es necesario comparar los últimos 2 bits 
        tenemos los siguientes casos para los LSB de la trama[16]:
        00--- trama de información
        01 -- trama de supervisión
        10 -- trama de información
        11 -- trama no numerada
        
        por eso decimos que si valen 1 es una trama de supervisión, si vale 3 es no numerada
        y en cualquier otro caso, es de información
        
        dependiendo del tipo de trama tendrán distintas longitudes:
        estas serán sus longitudes:
        Trama de información -- 2 bytes (16 bits)
        Trama de supervisión -- 2 bytes (16 bits)
        Trama no numerada -- 1 byte
